	...
	$(document).ready(function () {
                ...
                renderToolbar: function(toolBar)
                {
                    ...
                    FileExcelButton.click(function(event){
					  $('#loadingDiv').show();
                       ...
                       var selectedId="<?php echo $ivalue; ?>";
                        $.ajax({
                            url: "<?php echo base_url().'hr/kbi-rev/M_KBI/DOWNLOAD_FILE_BLM_ISI';?>",
                            type: 'POST',
                            data: {json: JSON.stringify(selectedId)},
                            dataType: 'json',
                            success: function (data) {
            ...
            {	width: '90%',
                ...
                renderToolbar: function(toolBar)
                {
                    ...
                    var btnSentNotif = '<button id="send_notifbutton" type="button" value="" title="Kirim Notifikasi ke WTONMobile" style="cursor:pointer;padding: 3px; margin: 2px;float:left;"><i class="fa fa-mobile fa-lg fa-fw">&nbsp;</i>Kirim Notifikasi</button>';
                    var btnFileExcel = '<button id="file_excel" type="button" value="" title="Download File Xls" style="cursor:pointer;padding: 3px; margin: 2px;float:left;"><i class="fa fa-file-excel-o  fa-lg fa-fw">&nbsp;</i>Download File</button>';
                    var btnFileExcelNilai = '<button id="file_excel_nilai" type="button" value="" title="Download Rekap Nilai" style="cursor:pointer;padding: 3px; margin: 2px;float:left;"><i class="fa fa-file-excel-o  fa-lg fa-fw">&nbsp;</i>Download Nilai</button>';
                    
					var loadingDiv = "<div style='margin: 2px;float:left;' id='loadingDiv' ><img src='<?php echo base_url(); ?>assets/images/lg.rotating-squares-preloader-gif.gif' width='35px' height='35px;' /></div>";
                    var pdfButton = $(btnPdf);
                    var SendNotifButton = $(btnSentNotif);var FileExcelButton = $(btnFileExcel);var FileExcelNilaiButton = $(btnFileExcelNilai);
                    var loadingDivGif = $(loadingDiv);
                    pdfButton.jqxButton({theme:'ui-start', width:'130px', height:'35px'});
                    SendNotifButton.jqxButton({theme:'ui-start', width:'130px', height:'35px'});
                    FileExcelButton.jqxButton({theme:'ui-start', width:'130px', height:'35px'});
					FileExcelNilaiButton.jqxButton({theme:'ui-start', width:'130px', height:'35px'});
                    container.append(pdfButton);container.append(SendNotifButton);container.append(FileExcelButton);container.append(FileExcelNilaiButton);
                    toolBar.append(container);
                    $('#formLoaderGif').hide();
					...
					FileExcelNilaiButton.click(function(event){
						$('#loadingDiv').show();
                        $('#formLoaderGif').modal('show');
						var selectedId="<?php echo $ivalue; ?>";
						/*
						var selectedId="<?php echo $ivalue; ?>";
						 $.ajax({
                            url: "<?php echo base_url().'hr/kbi-rev/M_KBI/DOWNLOAD_FILE_NILAI';?>",
						    type: 'POST',
                            data: {json: JSON.stringify(selectedId)},
                            dataType: 'json',
                            success: function (data) {
						...
						*/
						...
						$.ajax({
							type: "GET",
							url: '<?php echo base_url()."hr/kbi-rev/M_KBI/DOWNLOAD_FILE_NILAI/?ibl=".$ivalue;?>',
							async: true,
							success: function(data) {
								if(data!==null){
									var jsonData = $.parseJSON(data);
                    ...
                    FileExcelButton.click(function(event){
                        ...
                        var selectedId="<?php echo $ivalue; ?>";
					    $.ajax({
                            url: "<?php echo base_url().'hr/kbi-rev/M_KBI/DOWNLOAD_FILE_SDH_ISI';?>",
                            type: 'POST',
                            data: {json: JSON.stringify(selectedId)},
                            dataType: 'json',
                            success: function (data) {
