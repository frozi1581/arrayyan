...
class M_index extends CI_Controller {
    ...
    public function contentManagement(){
		$params["title"]="Header Key Behaviour Index";
    ...
    public function getGridDataEdit($params="get"){
		...
		$query2='';
	     	if(isset($_GET["filterscount"]))$where = genJQWFilterSQL($this->initGridL()) ;
               ...
               $query="select * from (
                select src.st_date,src.end_date,src.bl,  src.EMPLOYEE_ID,src.EMPLOYEE_NAME,src.ESELON,src.UNIT_KERJA,src.BAGIAN,src.IS_MANAGER,src.STATUS_RESPON,src.APP,src2.PERSENTASE from(
                    ...
                    left join (
                    select aa.*,bb.jml_nilai_masuk,round(nvl(bb.jml_nilai_masuk,0)/aa.jml_penilai*100,2) persentase from(
                select bl,st_date,end_date,employee_id,count(employee_id)jml_penilai
                from( 
                select distinct x.penilai,x.bl,x.st_date,x.end_date,x.employee_id,employee_id2 from (select employee_id||','||employee_id_atasan||','||employee_id_peer1||','||employee_id_peer2||','||
                employee_id_sub1||','||employee_id_sub2||','||
                employee_id_sub3 penilai,bl,st_date,end_date,employee_id from kbi_d)x 
                inner join kbi_kp_ik y on x.bl=y.bl and x.st_date=y.st_date and x.end_date=y.end_date and x.employee_id=y.employee_id
                where instr(x.penilai,y.employee_id2)>0
                order by x.employee_id
                 )src
                 group by bl,st_date,end_date,employee_id
                )aa
                left join
                (
                select bl,st_date,end_date,employee_id,count(employee_id)jml_nilai_masuk
                from( 
                select distinct x.penilai,x.bl,x.st_date,x.end_date,x.employee_id,employee_id2 from (select employee_id||','||employee_id_atasan||','||employee_id_peer1||','||employee_id_peer2||','||
                employee_id_sub1||','||employee_id_sub2||','||
                employee_id_sub3 penilai,bl,st_date,end_date,employee_id from kbi_d)x 
                inner join kbi_kp_ik y on x.bl=y.bl and x.st_date=y.st_date and x.end_date=y.end_date and x.employee_id=y.employee_id
                where instr(x.penilai,y.employee_id2)>0 and length(trim(y.ik_id))>1
                order by x.employee_id
                 )src
                 group by bl,st_date,end_date,employee_id
                 )bb on aa.bl=bb.bl and aa.st_date=bb.st_date and aa.end_date=bb.end_date and aa.employee_id=bb.employee_id 
                    )src2 on src.bl=src2.bl and src.st_date=src2.st_date and src.end_date=src2.end_date and src.employee_id=src2.employee_id    
                    )srcx                
                 where BL='".$a_params[0]."' and to_char(st_date,'dd-mm-yyyy')='".$a_params[1]."' and  to_char(end_date,'dd-mm-yyyy')='".$a_params[2]."'   ".$where;
                $arrParamData = array();
		$rst=$conn->PageExecute($query,$recPerPage,$currPage,$arrParamData);
                if($rst)
		{	$total_rows = $this->adodbconn->gettotalrec($conn, $query, $arrParamData);
			if($rst->recordCount()>0){
				while(!$rst->EOF)
				{
					$arrData[] = array(
                                            ...
                                            'STATUS_RESPON' =>  $rst->fields["PERSENTASE"],
                                            'APP' =>  $rst->fields["APP"],
                                            'FLAG_DATA' => "0"
                                            );	
    ...
    public function V_KBI_RESULT(){
        $json      = utf8_encode($_POST['json']); // Don't forget the encoding
        $data      = json_decode($json);
        //[0]txtBL,[1]txtST_DATE,[2]txtEND_DATE,[3]EMPLOYEE_ID,[4]IS_MANAGER
        $sql="select a.employee_id,trim(a.first_title||' '||a.first_name||' '||a.last_name||' '||a.last_title
            )nama_dinilai,b.ket ket_pat,c.ket ket_gas,d.ket ket_jbt
            from hrms.personal a
            inner join hrms.tb_pat b on a.kd_pat=b.kd_pat
            inner join hrms.tb_gas c on a.kd_gas=c.kd_gas
            inner join hrms.tb_jbt d on a.kd_jbt=d.kd_jbt
            where employee_id='".$data[3]."'";
        $getData = $this->db->query($sql)->result_array();
        if($this->db->query($sql)->num_rows()>0){
            foreach ($getData as $row){
                $params["NIP_DINILAI"]=$row["EMPLOYEE_ID"];
                $params["NAMA_DINILAI"]=$row["NAMA_DINILAI"];
                $params["KET_PAT_DINILAI"]=$row["KET_PAT"];
                $params["KET_GAS_DINILAI"]=$row["KET_GAS"];
                $params["KET_JBT_DINILAI"]=$row["KET_JBT"];
                $params["BL"]=$data[0];
            }
        }   
        $sql="select a.employee_id2,trim(c.first_title||' '||c.first_name||' '||c.last_name||' '||c.last_title
            ||'('||trim(f.ket)||','||trim(e.ket)||','||trim(d.ket))nama_penilai,
            upper(b.jenis_kriteria_penilaian)jenis_kriteria_penilaian,upper(b.kriteria_penilaian)kriteria_penilaian,
            nvl(a.ik_skor,0)ik_skor,nvl(a.ik_bobot,0)ik_bobot,
            case a.employee_id2
            when g.employee_id_atasan then 'ATASAN'
            when g.employee_id_peer1 then 'PEER1'
            when g.employee_id_peer2 then 'PEER2'
            when g.employee_id_sub1 then 'SUB1'
            when g.employee_id_sub2 then 'SUB2'
            when g.employee_id_sub3 then 'SUB3'
            when g.employee_id then 'DIRI SENDIRI'
            ELSE ''  end  as status_penilai,
            case a.employee_id2
            when g.employee_id_atasan then
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='ATASAN' and is_manager='".$data[4]."'
                )
            when g.employee_id_peer1 then 
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='PEER1' and is_manager='".$data[4]."'
                )
            when g.employee_id_peer2 then 
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='PEER2' and is_manager='".$data[4]."'
                )
            when g.employee_id_sub1   then 
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='SUB' and is_manager='".$data[4]."'
                )
            when g.employee_id_sub2 then 
                    (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='SUB' and is_manager='".$data[4]."'
                )
            when g.employee_id_sub3 then 
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='SUB' and is_manager='".$data[4]."'
                )
            when g.employee_id then
                (select nilai_bobot_persen from hrms.kbi_bobot 
                where tgl_berlaku=b.tgl_berlaku
                and jenis_penilai='SELF' and is_manager='".$data[4]."'
                )
            ELSE 0    
            end  as persen_bobot
            from hrms.kbi_kp_ik a
            inner join (select tgl_berlaku,kp_id,jenis_kriteria_penilaian,kriteria_penilaian from hrms.kbi_kp_h 
            where tgl_berlaku<=to_date('".$data[2]."','dd-mm-yyyy') and is_manager='".$data[4]."')b
            on b.kp_id=a.kp_id
            inner join hrms.personal c on a.employee_id2=c.employee_id
            inner join hrms.tb_pat d on c.kd_pat=d.kd_pat
            inner join hrms.tb_gas e on c.kd_gas=e.kd_gas
            inner join hrms.tb_jbt f on c.kd_jbt=f.kd_jbt
            inner join hrms.kbi_d g on a.bl=g.bl and a.st_date=g.st_date and a.end_date=g.end_date and a.employee_id=g.employee_id
            where  a.employee_id='".$data[3]."' and a.bl='".$data[0]."' and to_date('".$data[1]."','dd-mm-yyyy')=a.st_date
            and to_date('".$data[2]."','dd-mm-yyyy')=a.end_date 
            order by a.employee_id2,a.kp_id";
        $arrParamData=array();
        $getData = $this->db->query($sql)->result_array();
        ...
        if($this->db->query($sql)->num_rows()>0){
            $employee_id2='';
            foreach ($getData as $row){
                if($employee_id2<>$row["EMPLOYEE_ID2"]){
                   if(isset($arrSoal)){
                       if(trim($status_penilai)<>''){
                         $arrMasterFromDB['GET_KBI'][]=array("status"=>true,
                                "BL"=>$params["BL"],
                                "NIP_DINILAI"=>$params["NIP_DINILAI"],
                                "NAMA_DINILAI"=>$params["NAMA_DINILAI"],
                                "KET_PAT_DINILAI"=>$params["KET_PAT_DINILAI"],
                                "KET_GAS_DINILAI"=>$params["KET_GAS_DINILAI"],
                                "KET_JBT_DINILAI"=>$params["KET_JBT_DINILAI"],
                                "PENILAI"=>$penilai,
                                "SOAL"=>$arrSoal,
                                "SOAL_IK"=>$arrSoal_IK,
                                "IK_SKOR"=>$arrIKSkor,
                                ...
                                "P_BOBOT"=>$persen_bobot
                                );
                       }
                    }
                    $employee_id2=$row["EMPLOYEE_ID2"];
                    $penilai=$row["NAMA_PENILAI"];
                    $status_penilai=$row["STATUS_PENILAI"];
                    $persen_bobot=$row["PERSEN_BOBOT"];
                    $arrSoal = array();
                    $arrIKSkor= array();
                    $arrIKBobot= array();
                    $arrSoal_IK = array();
                }
                array_push($arrSoal,$row["JENIS_KRITERIA_PENILAIAN"].':'.$row["KRITERIA_PENILAIAN"]);
                array_push($arrSoal_IK,$row["JENIS_KRITERIA_PENILAIAN"]);
                array_push($arrIKSkor,$row["IK_SKOR"]);
                array_push($arrIKBobot,$row["IK_BOBOT"]);
           }
            if(trim($status_penilai)<>''){
               $arrMasterFromDB['GET_KBI'][]=array("status"=>true,
                                "BL"=>$params["BL"],
    ...
    public function  HITUNG_RESPONSE(){
        $json      = utf8_encode($_POST['json']); // Don't forget the encoding
        $data      = json_decode($json);
         /* selectedId[0] = document.getElementById("txtBL").value;
               ...
               selectedId[3] = obj.EMPLOYEE_ID;*/
             
        $sql="select aa.*,bb.jml_nilai_masuk,round(nvl(bb.jml_nilai_masuk,0)/aa.jml_penilai*100,2) persentase from(
                select bl,st_date,end_date,employee_id,count(employee_id)jml_penilai
                from( 
                select distinct x.penilai,x.bl,x.st_date,x.end_date,x.employee_id,employee_id2 from (select employee_id||','||employee_id_atasan||','||employee_id_peer1||','||employee_id_peer2||','||
                employee_id_sub1||','||employee_id_sub2||','||
                employee_id_sub3 penilai,bl,st_date,end_date,employee_id from kbi_d)x 
                inner join kbi_kp_ik y on x.bl=y.bl and x.st_date=y.st_date and x.end_date=y.end_date and x.employee_id=y.employee_id
                where x.bl='012018' and x.st_date=to_date('01-08-2018','dd-mm-yyyy') and x.end_date=to_date('31-08-2018','dd-mm-yyyy')
                and instr(x.penilai,y.employee_id2)>0
                order by x.employee_id
                 )src
                 group by bl,st_date,end_date,employee_id
                )aa
                ...
                inner join kbi_kp_ik y on x.bl=y.bl and x.st_date=y.st_date and x.end_date=y.end_date and x.employee_id=y.employee_id
                where x.bl='012018' and x.st_date=to_date('01-08-2018','dd-mm-yyyy') and x.end_date=to_date('31-08-2018','dd-mm-yyyy')
                and instr(x.penilai,y.employee_id2)>0 and length(trim(y.ik_id))>2
                order by x.employee_id
                 )src
                 group by bl,st_date,end_date,employee_id
                 )bb on aa.bl=bb.bl and aa.st_date=bb.st_date and aa.end_date=bb.end_date and aa.employee_id=bb.employee_id
                 ";
        $sql = "select aa.*,bb.jml_penilai_masuk,round(bb.jml_penilai_masuk/jml_penilai*100,2) persentase from (
            select bl,st_date,end_date,employee_id,count(employee_id)jml_penilai
            from(
            select  bl,st_date,end_date,employee_id,employee_id2
             from hrms.kbi_kp_ik
             where  bl='".$data[0]."' and st_date=to_date('".$data[1]."','dd-mm-yyyy') and end_date=to_date('".$data[2]."','dd-mm-yyyy')
             and employee_id='".$data[3]."'    
             group by bl,st_date,end_date,employee_id,employee_id2
             order by employee_id
             )src
             group by bl,st_date,end_date,employee_id
             )aa
             left join 
             (select bl,st_date,end_date,employee_id,count(employee_id)jml_penilai_masuk
            from(
            select  bl,st_date,end_date,employee_id,employee_id2
             from hrms.kbi_kp_ik
             where  length(trim(ik_id))>2
             and bl='".$data[0]."' and st_date=to_date('".$data[1]."','dd-mm-yyyy') and end_date=to_date('".$data[2]."','dd-mm-yyyy') and employee_id='".$data[3]."' 
             group by bl,st_date,end_date,employee_id,employee_id2
             order by employee_id
             )src
             group by bl,st_date,end_date,employee_id)bb
             on aa.bl=bb.bl and aa.st_date=bb.st_date and aa.end_date=bb.end_date and aa.employee_id=bb.employee_id";
        $getData = $this->db->query($sql)->result_array();
    ...
    public function  APPROVE(){
       ...
       //------------------------------------------------------- update kbi_h -----------------------------------------------------------
            ...
            if($mysql_db->query($sql_m2)->num_rows()<1){
              $sql_m2="insert into kbi_d(bl,st_date,end_date,employee_id,eselon,employee_id_atasan,employee_id_peer1,employee_id_peer2,"
               ...
               . $row['EMPLOYEE_ID_SUB2']."','".$row['EMPLOYEE_ID_SUB3']."','".$row['IS_MANAGER']."','".$row['CREATED_BY']."',"
                ...
                $sql_m2="update kbi_d set employee_id_atasan='". $row['EMPLOYEE_ID_ATASAN']."',employee_id_peer1='".$row['EMPLOYEE_ID_PEER1']."',employee_id_peer2='"
                     .$row['EMPLOYEE_ID_PEER2']."',employee_id_sub1='".$row['EMPLOYEE_ID_SUB1']."',employee_id_sub2='".$row['EMPLOYEE_ID_SUB2']."',employee_id_sub3='".$row['EMPLOYEE_ID_SUB3']."' "
                     ." where bl='".$row['BL']."' and st_date=str_to_date('".$row['ST_DATE']."','%d/%m/%Y') and end_date=str_to_date('".$row['END_DATE']."','%d/%m/%Y')
                    and employee_id='".$row['EMPLOYEE_ID']."'";     
            }
        ...
        foreach ($getData as $row){
            ...
            if($mysql_db->query($sql_m2)->num_rows()<1){
                $sql_m2="insert into kbi_kp_d(tgl_berlaku,is_manager,no_urut,jenis_kriteria_penilaian,no_urut_ik,
                  ...
                  .$user."',curdate(),'".$row['IK_ID']."')";
                $mysql_db->query($sql_m2);
            }
        }
        //------------------------------------------------------- update kbi_kp_ik --------------------------------------------------------
        $sql="select bl,to_char(st_date,'dd/mm/yyyy')st_date,to_char(end_date,'dd/mm/yyyy')end_date,employee_id,kp_id,ik_id,employee_id2,created_by,
        ...
        $getData = $this->db->query($sql)->result_array();
        foreach ($getData as $row){
            $sql_m22="select bl from kbi_kp_ik where bl='". $row['BL']."' and st_date= str_to_date('".$row['ST_DATE']."','%d/%m/%Y')
                    and end_date= str_to_date('".$row['END_DATE']."','%d/%m/%Y') and employee_id='".$row['EMPLOYEE_ID']."' and employee_id2='".$row['EMPLOYEE_ID2']."' and kp_id='".$row['KP_ID']."'";
           if($mysql_db->query($sql_m22)->num_rows()<1){
    ...
    public function KBIResume($recId="get"){
        $a_params= explode('_', $recId);
        $params["title"]="Hasil Penilaian KBI";
      ...
      //  $this->render_view('hr/kbi/v_resume', $params);
         $this->render_view('hr/kbi/spyder_chart', $params);
    }
    ...
    public  function f_chart_peserta(){
       //echo $_POST['desc1'];
       /* foreach ($_POST as $key => $value) {
            if(length(trim(htmlspecialchars($value))>0){
                if(strpos(htmlspecialchars($key), 'desc')>0){
                    $arrData[] = array(
        ...
        */
        ...
        $params["title"]="Header Key Behaviour Index Chart";
        $params["data"]=$arrData;
        $params["jml_data"]=$x;
        $params["fc_bl"]=$_POST['fc_bl'];
        $params["fc_st"]=$_POST['fc_st'];
        $params["fc_end"]=$_POST['fc_end'];
        $params["fc_nama"]=$_POST['fc_nama'];
        $this->render_view('hr/kbi/spyder_chart', $params);
    }
        ...
        try {
            ...
            for ($row = 2; $row <= $highestRow; $row++){                  //  Read a row of data into an array                 
                $rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,
                                                ...
                                                TRUE,
                                                FALSE);
                $status='Proses Upload Data Pegawai :'.($row-1).' Periode Penilian :'.$ST_DATE.' s/d '.$END_DATE;
    ...
    public function f_download_kbi(){
        $BL = $_POST['txtBL_M'];
        $ST_DATE = $_POST['txtST_DATE_M'];
        $END_DATE = $_POST['txtEND_DATE_M'];  
        $TYPE = $_POST['txtTypeReport'];  
        if($TYPE==='R1'){
             $sql="select employee_id,max(nama_self)nama_pegawai,
            is_manager,max(kriteria_penilaian)kriteria_penilaian,sum(bobot_self)bobot_self,
            max(atasan)atasan,max(nama_atasan)nama_atasan,sum(bobot_atasan)bobot_atasan,max(peer1)peer1,max(nama_peer1)nama_peer1,
            sum(bobot_peer1)bobot_peer1,max(peer2)peer2,max(nama_peer2)nama_peer2,
            sum(bobot_peer2)bobot_peer2,
            max(sub1)sub1,max(nama_sub1)nama_sub1,
            sum(bobot_sub1)bobot_sub1,max(sub2)sub2,max(nama_sub2)nama_sub2,
            sum(bobot_sub2)bobot_sub2,max(sub3)sub3,max(nama_sub3)nama_sub3,sum(bobot_sub3)bobot_sub3
             from (
            select a.employee_id,a.is_manager,b.employee_id2,
            c.kriteria_penilaian,c.kp_id,
            b.ik_bobot,
                        case b.employee_id2 
                        when a.employee_id then 'SELF'
                        when a.employee_id_atasan then 'ATASAN'
                        when a.employee_id_peer1 then 'PEER1'
                        when a.employee_id_peer2 then 'PEER2'
                        when a.employee_id_sub1 then 'SUB1'
                        when a.employee_id_sub2 then 'SUB2'
                        when a.employee_id_sub3 then 'SUB3'
                        else ''
                        end as status,
                        case  b.employee_id2
                        when a.employee_id then b.employee_id2
                        end as self,
                        case  b.employee_id2
                        when a.employee_id then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_self,
                        case  b.employee_id2
                        when a.employee_id then b.ik_bobot
                        end as bobot_self,
                        case  b.employee_id2
                        when a.employee_id_atasan then b.employee_id2
                        end as atasan,
                        case  b.employee_id2
                        when a.employee_id_atasan then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_atasan,
                        case  b.employee_id2
                        when a.employee_id_atasan then b.ik_bobot
                        end as bobot_atasan,
                        case  b.employee_id2
                        when a.employee_id_peer1 then b.employee_id2
                        end as peer1,
                         case  b.employee_id2
                        when a.employee_id_peer1 then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_peer1,
                        case  b.employee_id2
                        when a.employee_id_peer1 then b.ik_bobot
                        end as bobot_peer1,
                        case  b.employee_id2
                        when a.employee_id_peer2 then b.employee_id2
                        end as peer2,
                        case  b.employee_id2
                        when a.employee_id_peer2 then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_peer2,
                        case  b.employee_id2
                        when a.employee_id_peer2 then b.ik_bobot
                        end as bobot_peer2,
                        case  b.employee_id2
                        when a.employee_id_sub1 then b.employee_id2
                        end as sub1,
                        case  b.employee_id2
                        when a.employee_id_sub1 then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_sub1,

                        case  b.employee_id2
                        when a.employee_id_sub1 then b.ik_bobot
                        end as bobot_sub1,
                        case  b.employee_id2
                        when a.employee_id_sub2 then b.employee_id2
                        end as sub2,
                        case  b.employee_id2
                        when a.employee_id_sub2 then b.ik_bobot
                        end as bobot_sub2,
                        case  b.employee_id2
                        when a.employee_id_sub2 then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_sub2,
                        case  b.employee_id2
                        when a.employee_id_sub3 then b.employee_id2
                        end as sub3,
                        case  b.employee_id2
                        when a.employee_id_sub3 then trim(d.first_title||' '||d.first_name||' '||d.last_name||' '||d.last_title)
                        end as nama_sub3,
                        case  b.employee_id2
                        when a.employee_id_sub3 then b.ik_bobot
                        end as bobot_sub3
                        from hrms.kbi_d a
                        inner join hrms.kbi_kp_ik b on a.bl=b.bl and a.st_date=b.st_date and a.end_date=b.end_date and a.employee_id=b.employee_id
                        inner join   hrms.kbi_kp_h c on b.kp_id=c.kp_id and c.tgl_berlaku between b.st_date and b.end_date
                        left join hrms.personal d on b.employee_id2=d.employee_id
                        where a.bl='".$BL."' and a.st_date=to_date('".$ST_DATE."','dd/mm/yyyy') and  a.end_date=to_date('".$END_DATE."','dd/mm/yyyy')
                         order by a.employee_id,b.employee_id2,c.kp_id
                         )src where length(trim(status))>1 and status is not null
                         group by employee_id,is_manager,kp_id
                         order by employee_id,kp_id";
            $fileName='download/tabulasi_KBI.xlsx';
            $header= array('NIP','NAMA PEGAWAI','STATUS','KRITERIA PENILAIAN','BOBOT NILAI SENDIRI',
                'NIP ATASAN','NAMA ATASAN','BOBOT NILAI ATASAN','NIP PEER1','NAMA PEER1','BOBOT NILAI PEER1',
                'NIP PEER2','NAMA PEER2','NILAI PEER2','NIP BAWAHAN1','NAMA BAWAHAN1','NILAI BAWAHAN1',
                'NIP BAWAHAN2','NAMA BAWAHAN2','NILAI BAWAHAN2','NIP BAWAHAN3','NAMA BAWAHAN3','NILAI BAWAHAN3','NILAI KBI');
        }else{
            $sql="select penilai,
            trim(b.first_title||' '||b.first_name||' '||b.last_name||' '||b.last_title)nama_penilai ,
                d.ket lokasi_kerja,dinilai,
            trim(c.first_title||' '||c.first_name||' '||c.last_name||' '||c.last_title)nama_dinilai,
           a.keterangan
            from (
            select a.bl,a.st_date,a.end_date,a.dinilai,a.penilai,
            case  a.penilai  when b.employee_id_atasan then 'BAWAHAN' 
            when b.employee_id_peer1   then 'PEER'
            when  b.employee_id_peer1  then 'PEER'
            when b.employee_id_sub1 then 'ATASAN'
            when b.employee_id_sub2 then 'ATASAN'
            when b.employee_id_sub3 then 'ATASAN'
            when b.employee_id then 'DIRI SENDIRI'
            end keterangan
            from (
            select  distinct bl,st_date,end_date,employee_id dinilai,employee_id2 penilai from hrms.kbi_kp_ik 
            where length(trim(ik_id))<1 or ik_id is null)a
            inner join hrms.kbi_d b on  a.bl=b.bl and a.st_date = b.st_date and a.end_date=b.end_date and a.dinilai=b.employee_id 
            )a
           inner join hrms.personal b on a.penilai=b.employee_id
           inner join hrms.personal c on a.dinilai=c.employee_id
           inner join tb_pat d on b.kd_pat=d.kd_pat 
                    where length(trim(keterangan))>1 and  a.bl='".$BL."' and a.st_date=to_date('".$ST_DATE."','dd/mm/yyyy') and  a.end_date=to_date('".$END_DATE."','dd/mm/yyyy') order by penilai ";
       
            $fileName='download/Daftar_Pegawai_Belum_Isi_KBI.xlsx';
            $header= array('NIP PENILAI','NAMA PEGAWAI PENILAI','AREA KERJA','NIP DINILAI','NAMA PEGAWAI DINILAI','KETERANGAN PEGAWAI DINILAI');
        }
        $objPHPExcel = new PHPExcel();
        
        if($TYPE==='R2'){
            $objPHPExcel->setActiveSheetIndex(0);
            $objPHPExcel->getActiveSheet()->setTitle("Belum Mengisi Nilai");
        }else{
            $objPHPExcel->setActiveSheetIndex(0);
            $objPHPExcel->getActiveSheet()->setTitle("Tabulasi Penilaian");
        }
        ...
        $row=2;
        $getData = $this->db->query($sql)->result_array();
        foreach ($getData as $fld){
            //$APP1 = $rowx['APP1'];
            foreach ($header as $opt)
            {  $col = 0;$sub_bobot=0;
                if($TYPE==='R1'){
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['EMPLOYEE_ID']);$col++;
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_PEGAWAI']);$col++;
                        $cntbwh=0;$bobot_bwh=0;
                         if($fld['IS_MANAGER']==='0'){
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, 'NON MANAGER');$col++;
                            $sub_bobot=$fld['BOBOT_SELF']+$fld['BOBOT_ATASAN'];
                            if(strlen(trim($fld['PEER1']))>0)$sub_bobot+=$fld['BOBOT_PEER1'];
                            if(strlen(trim($fld['PEER1']))>0)$sub_bobot+=$fld['BOBOT_PEER2'];
                         }else{
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, 'MANAGER');$col++;
                            $sub_bobot=$fld['BOBOT_SELF']+$fld['BOBOT_ATASAN'];
                            if(strlen(trim($fld['PEER1']))>0)$sub_bobot+=$fld['BOBOT_PEER1'];
                            if(strlen(trim($fld['PEER1']))>0)$sub_bobot+=$fld['BOBOT_PEER2'];
                            if(strlen(trim($fld['SUB1']))>0 and $fld['BOBOT_SUB1']>0){$bobot_bwh+=$fld['BOBOT_SUB1'];$cntbwh+=1;}
                            if(strlen(trim($fld['SUB2']))>0 and $fld['BOBOT_SUB2']>0){$bobot_bwh+=$fld['BOBOT_SUB2'];$cntbwh+=1;}
                            if(strlen(trim($fld['SUB3']))>0 and $fld['BOBOT_SUB3']>0){$bobot_bwh+=$fld['BOBOT_SUB3'];$cntbwh+=1;}
                            if ($cntbwh>0)$sub_bobot+=round(($bobot_bwh/$cntbwh),2);
                         }
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['KRITERIA_PENILAIAN']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_SELF']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['ATASAN']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_ATASAN']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_ATASAN']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['PEER1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_PEER1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_PEER1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['PEER2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_PEER2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_PEER2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['SUB1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_SUB1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_SUB1']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['SUB2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_SUB2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_SUB2']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['SUB3']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_SUB3']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['BOBOT_SUB3']);$col++;
                         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $sub_bobot);$col++;
                }else{
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['PENILAI']);$col++;
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_PENILAI']);$col++;
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['LOKASI_KERJA']);$col++;
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['DINILAI']);$col++;
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_DINILAI']);$col++;
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['KETERANGAN']);$col++;
                }
            }
            $row++;
        }
        ...
        if($TYPE==='R1'){
            ...
            $sql="select  employee_id,max(nama_self)nama_pegawai,
            is_manager,sum(bobot_self)bobot_self,
            max(atasan)atasan,max(nama_atasan)nama_atasan,sum(bobot_atasan)bobot_atasan,max(peer1)peer1,max(nama_peer1)nama_peer1,
            sum(bobot_peer1)bobot_peer1,max(peer2)peer2,max(nama_peer2)nama_peer2,
            sum(bobot_peer2)bobot_peer2,
            max(sub1)sub1,max(nama_sub1)nama_sub1,
            ...
            b.ik_bobot,
                        ...
                        where a.bl='".$BL."' and a.st_date=to_date('".$ST_DATE."','dd/mm/yyyy') and  a.end_date=to_date('".$END_DATE."','dd/mm/yyyy')
                         order by a.employee_id,b.employee_id2,c.kp_id)
                         src where length(trim(status))>1 and status is not null
                         group by employee_id,is_manager
                         order by employee_id";
            ...
            $objPHPExcel->getActiveSheet()->setTitle("Summary KBI");
            $header= array('NIP','NAMA PEGAWAI','STATUS','PERSEN NILAI MASUK','KBI INDIVIDU','STANDAR KBI','KBI FIT','HASIL PENILAIAN');
            $col=0;$row=1;
            foreach ($header as $opt) {
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $opt);$col++;
            }
            ...
            $sql2="select is_manager,max(no_urut)jml_soal from hrms.kbi_kp_h where 
                tgl_berlaku between to_date('".$ST_DATE."','dd/mm/yyyy') and to_date('".$END_DATE."','dd/mm/yyyy')
                group by tgl_berlaku,is_manager";
            $getData = $this->db->query($sql2)->result_array();
            $jml_soal_mnj=0;$jml_soal_non=0;
            foreach ($getData as $fld){
               if($fld['IS_MANAGER']==='1')  $jml_soal_mnj=$fld['JML_SOAL'];
               if($fld['IS_MANAGER']==='0')  $jml_soal_non=$fld['JML_SOAL'];
            }
            $getData = $this->db->query($sql)->result_array();
            //------------------------------------------------------------------------------------------------------------------
            foreach ($getData as $fld){
                $jml_penilai=1;$jml_penilai_isi=0;$jml_bobot_bwh=0;$jml_penilai_bwh=0;$tot_bobot=0;$r_bobot_bwh=0;$persen_isi=0;$std_kbi=3;$kbi_fit=0;$hasil="";
                if($fld['BOBOT_SELF']>0){$tot_bobot+=$fld['BOBOT_SELF'];$jml_penilai_isi++;}
                if(strlen(trim($fld['ATASAN']))>0){$jml_penilai++;if($fld['BOBOT_ATASAN']>0){$tot_bobot+=$fld['BOBOT_ATASAN'];$jml_penilai_isi++;}}
                if(strlen(trim($fld['PEER1']))>0){$jml_penilai++;if($fld['BOBOT_PEER1']>0){$tot_bobot+=$fld['BOBOT_PEER1'];$jml_penilai_isi++;}}
                if(strlen(trim($fld['PEER2']))>0){$jml_penilai++;if($fld['BOBOT_PEER2']>0){$tot_bobot+=$fld['BOBOT_PEER2'];$jml_penilai_isi++;}}
                if(strlen(trim($fld['SUB1']))>0){$jml_penilai++;$jml_bobot_bwh+=$fld['BOBOT_SUB1'];if($fld['BOBOT_SUB1']>0){$jml_penilai_bwh++;$jml_penilai_isi++;}}
                if(strlen(trim($fld['SUB2']))>0){$jml_penilai++;$jml_bobot_bwh+=$fld['BOBOT_SUB2'];if($fld['BOBOT_SUB2']>0){$jml_penilai_bwh++;$jml_penilai_isi++;} }
                if(strlen(trim($fld['SUB3']))>0){$jml_penilai++;$jml_bobot_bwh+=$fld['BOBOT_SUB3'];if($fld['BOBOT_SUB3']>0){$jml_penilai_bwh++;$jml_penilai_isi++;} }
                if($jml_penilai_bwh>0)$r_bobot_bwh=$jml_bobot_bwh/$jml_penilai_bwh;
                $tot_bobot+=$r_bobot_bwh;
                if($jml_penilai_isi>0){
                    if($fld['IS_MANAGER']==='0')$tot_bobot=$tot_bobot/$jml_soal_mnj;
                    if($fld['IS_MANAGER']==='1')$tot_bobot=$tot_bobot/$jml_soal_non;
                    $persen_isi=$jml_penilai_isi/$jml_penilai*100;
                    $kbi_fit=($tot_bobot/$std_kbi)*100;
                    if($kbi_fit<80)$hasil="Under Expectation";
                    if($kbi_fit>=80 and $kbi_fit<90)$hasil="Meet Expectation";
                    if($kbi_fit>=90 and $kbi_fit<95)$hasil="Above Expectation";
                    if($kbi_fit>=95)$hasil="Outstanding";
                }
                $col = 0;
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['EMPLOYEE_ID']);$col++;
                 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $fld['NAMA_PEGAWAI']);$col++;
                 if($fld['IS_MANAGER']==='0') {$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, 'NON MANAGER');$col++;}
                 if($fld['IS_MANAGER']==='1'){ $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, 'MANAGER');$col++;}
                  $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, round($persen_isi,2));$col++;
                 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, round($tot_bobot,2));$col++;
                 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $std_kbi);$col++;
                 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, round($kbi_fit,2));$col++;
                 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $hasil);$col++;
                $row++;
            }
            //------------------------------------------------------------------------------------------------------------------
        }
