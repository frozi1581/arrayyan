...
(function(b) {
    ...
    b.extend(b.jqx._jqxGrid.prototype, {
        ...
        _rendercelltexts: function() {
            if (b.jqx.credits !== "12F129D4-0E1B-44B8-9BBB-BB4CF78CC6BA") {
        ...
        ensurerowvisible: function(j) {
            ...
            if (this.groupable && this.groups.length > 0) {
                var l = this.getdisplayrows();
                var m = 0;
                for (var f = 0; f < l.length; f++) {
                    var q = l[f];
                    if (q.boundindex === j) {
                        var n = this.vScrollInstance.value;
                        var o = this._gettableheight() - this.rowsheight;
                        if (m < n || m > n + o) {
                            this.scrolltop(m - this.rowsheight)
                        }
                        if (m === n) {
                            this.scrolltop(m - this.rowsheight)
                        }
                    ...
                    if (this.details[f]) {
                        if (this.details[f].rowdetailshidden == false) {
                            m += this.details[f].rowdetailsheight
                        }
                    }
                }
                return true
            ...
            if (this._pageviews[g]) {
                var m = this._pageviews[g].top;
                var k = m + d * this.rowsheight;
                if (this.rowdetails) {
                    for (var f = e * g; f < j; f++) {
                        if (this.details[f]) {
                            if (this.details[f].rowdetailshidden == false) {
        ...
        _updatecolumnwidths: function() {
            ...
            var o = this;
            var r = 0;
            b.each(this.columns.records, function(v, z) {
                var x = b(this.element);
                ...
                if (this.width.toString().indexOf("%") != -1 || this._percentagewidth != undefined) {
                    if (this._percentagewidth != undefined) {
                        y = parseFloat(this._percentagewidth) * f / 100
                    } else {
                        y = parseFloat(this.width) * f / 100
                    }
                    q = true
                }
                if (e > 0 && this.columntype !== "adaptive") {
                    ...
                    if (this.width != "auto" && !this._width && !q) {
                        if (parseInt(x[0].style.width) != this.width) {
                            x.width(this.width)
                        }
                    } else {
                        if (q) {
                            if (y < this.minwidth && this.minwidth != "auto") {
                                y = this.minwidth;
                                this.width = y
                            }
                            if (y > this.maxwidth && this.maxwidth != "auto") {
                                y = this.maxwidth;
                                this.width = y
                            }
                            if (parseInt(x[0].style.width) != y) {
                                x.width(y);
                                this.width = y
                            }
                        } else {
                            var w = Math.floor(p * (this.text.length / n.length));
                            r += w;
                            if (p - r < 3 && p != r && p - r > 0.01) {
                                ...
                                if (v == u.columns.records.length - 1) {
                                    if (r < p) {
                                        w += (p - r)
                                    }
                                }
        ...
        _rendervisualcell: function(C, j, r, l, w, A, k, s, d, i, v, p) {
            var f = null;
            ...
            if (this.selectionmode.indexOf("cell") != -1) {
                ...
                if (this.virtualmode || (this.groupable && this.groups.length > 0 && this.pageable)) {
                    r = this._iscellselected(true, d, g.datafield)
                }
            }
            if (g.cellclassname != "" && g.cellclassname) {
                if (typeof g.cellclassname == "string") {
                    ...
                    var o = g.cellclassname(this.getboundindex(d), g.datafield, cellvalue, d.bounddata);
                    if (o) {
                        z += " " + o
                    }
                }
            }
            var q = this.showsortcolumnbackground && this.sortcolumn && g.displayfield == this.sortcolumn;
            if (this.sortmode === "many" && this.showsortcolumnbackground) {
                var h = this.getsortcolumns();
                for (var u = 0; u < h.length; u++) {
                    if (g.displayfield === h[u].dataField) {
                        q = true
                    }
                }
            }
            if (q) {
                z += " " + this.toTP("jqx-grid-cell-sort")
            }
            if (g.filter && this.showfiltercolumnbackground) {
                z += " " + this.toTP("jqx-grid-cell-filter")
            }
            if (d.bounddata.totalsrow) {
                z += " " + this.toTP("jqx-grid-cell-pinned")
            }
            ...
            if (this.altrows && d.group == undefined) {
                ...
                if (B >= this.altstart) {
                    if ((this.altstart + B) % (1 + this.altstep) == 0) {
                        if (!q) {
                            z += " " + this.toTP("jqx-grid-cell-alt")
                        } else {
                            z += " " + this.toTP("jqx-grid-cell-sort-alt")
                        }
                        if (g.filter && this.showfiltercolumnbackground) {
                            z += " " + this.toTP("jqx-grid-cell-filter-alt")
                        }
                        if (g.pinned && this.showpinnedcolumnbackground) {
                            z += " " + this.toTP("jqx-grid-cell-pinned-alt")
                        }
                    }
                }
            }
            if (i <= k) {
                if (A || this.rowdetails || (this.pageable && this.virtualmode)) {
                    var x = b(e);
            ...
            if (this.rowdetails && l) {
                if (w && !A) {
                    z += " " + this.toTP("jqx-grid-details-cell")
                } else {
                    if (A) {
                        z += " " + this.toTP("jqx-grid-group-details-cell")
                    }
                }
                if (this.showrowdetailscolumn) {
                    if (!this.rtl) {
                        if (d.group == undefined && i == k && !d.bounddata.totalsrow) {
                            var t = this.toThemeProperty("jqx-icon-arrow-down");
                            if (w) {
                                z += " " + this.toTP("jqx-grid-group-expand");
                                z += " " + t
                            } else {
                                z += " " + this.toTP("jqx-grid-group-collapse");
                                var t = this.toThemeProperty("jqx-icon-arrow-right");
                                z += " " + t
                            }
                            y = false;
                            e.title = "";
                        ...
                        if (d.group == undefined && i == s.cells.length - k - 1) {
                            var t = this.toThemeProperty("jqx-icon-arrow-down");
                            if (w) {
                                z += " " + this.toTP("jqx-grid-group-expand-rtl");
                                z += " " + t
                            } else {
                                z += " " + this.toTP("jqx-grid-group-collapse-rtl");
                                var t = this.toThemeProperty("jqx-icon-arrow-left");
                                z += " " + t
                            }
                            y = false;
                            e.title = "";
            ...
            if (r && y && i >= k) {
                z += " " + this.toTP("jqx-grid-cell-selected");
                z += " " + this.toTP("jqx-fill-state-pressed")
            }
            if (e.className != z) {
                e.className = z
            }
