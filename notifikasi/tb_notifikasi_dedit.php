...
$tb_notifikasi_d_edit = NULL; // Initialize page object first

class ctb_notifikasi_d_edit extends ctb_notifikasi_d {
	...
	// Page object name
	var $PageObjName = 'tb_notifikasi_d_edit';
	...
	function Page_Init() {
		...
		$this->notif_active->SetVisibility();
	...
	function LoadFormValues() {
		...
		if (!$this->notif_active->FldIsDetailKey) {
			$this->notif_active->setFormValue($objForm->GetValue("x_notif_active"));
		}
	...
	function RestoreFormValues() {
		...
		$this->notif_active->CurrentValue = $this->notif_active->FormValue;
	}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->notif_active->SetDbValueDef($rsnew, $this->notif_active->CurrentValue, NULL, $this->notif_active->ReadOnly);
...
// Create page object
if (!isset($tb_notifikasi_d_edit)) $tb_notifikasi_d_edit = new ctb_notifikasi_d_edit();

// Page init
$tb_notifikasi_d_edit->Page_Init();

// Page main
$tb_notifikasi_d_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_notifikasi_d_edit->Page_Render();
?>
...
var CurrentForm = ftb_notifikasi_dedit = new ew_Form("ftb_notifikasi_dedit", "edit");

// Validate form
ftb_notifikasi_dedit.Validate = function() {
...
ftb_notifikasi_dedit.Form_CustomValidate = 
...
ftb_notifikasi_dedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftb_notifikasi_dedit.Lists["x_employee_id"] = {"LinkField":"x_employee_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x_lok_kerja","x_bagian",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"v_personal"};
ftb_notifikasi_dedit.Lists["x_employee_id"].Data = "<?php echo $tb_notifikasi_d_edit->employee_id->LookupFilterQuery(FALSE, "edit") ?>";
ftb_notifikasi_dedit.AutoSuggests["x_employee_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tb_notifikasi_d_edit->employee_id->LookupFilterQuery(TRUE, "edit"))) ?>;
ftb_notifikasi_dedit.Lists["x_notif_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftb_notifikasi_dedit.Lists["x_notif_active"].Options = <?php echo json_encode($tb_notifikasi_d_edit->notif_active->Options()) ?>;
...
<?php $tb_notifikasi_d_edit->ShowPageHeader(); ?>
<?php
$tb_notifikasi_d_edit->ShowMessage();
?>
<form name="ftb_notifikasi_dedit" id="ftb_notifikasi_dedit" class="<?php echo $tb_notifikasi_d_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_notifikasi_d_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_notifikasi_d_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tb_notifikasi_d">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($tb_notifikasi_d_edit->IsModal) ?>">
...
<?php if ($tb_notifikasi_d->employee_id->Visible) { // employee_id ?>
	<div id="r_employee_id" class="form-group">
		<label id="elh_tb_notifikasi_d_employee_id" class="<?php echo $tb_notifikasi_d_edit->LeftColumnClass ?>"><?php echo $tb_notifikasi_d->employee_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $tb_notifikasi_d_edit->RightColumnClass ?>"><div<?php echo $tb_notifikasi_d->employee_id->CellAttributes() ?>>
...
<input type="hidden" data-table="tb_notifikasi_d" data-field="x_employee_id" data-page="1" name="x_employee_id" id="x_employee_id" value="<?php echo ew_HtmlEncode($tb_notifikasi_d->employee_id->CurrentValue) ?>">
...
<?php if ($tb_notifikasi_d->bagian->Visible) { // bagian ?>
	<div id="r_bagian" class="form-group">
		<label id="elh_tb_notifikasi_d_bagian" for="x_bagian" class="<?php echo $tb_notifikasi_d_edit->LeftColumnClass ?>"><?php echo $tb_notifikasi_d->bagian->FldCaption() ?></label>
		<div class="<?php echo $tb_notifikasi_d_edit->RightColumnClass ?>"><div<?php echo $tb_notifikasi_d->bagian->CellAttributes() ?>>
...
<input type="hidden" data-table="tb_notifikasi_d" data-field="x_bagian" data-page="1" name="x_bagian" id="x_bagian" value="<?php echo ew_HtmlEncode($tb_notifikasi_d->bagian->CurrentValue) ?>">
...
<?php if ($tb_notifikasi_d->lokasi_kerja->Visible) { // lokasi_kerja ?>
	<div id="r_lokasi_kerja" class="form-group">
		<label id="elh_tb_notifikasi_d_lokasi_kerja" for="x_lokasi_kerja" class="<?php echo $tb_notifikasi_d_edit->LeftColumnClass ?>"><?php echo $tb_notifikasi_d->lokasi_kerja->FldCaption() ?></label>
		<div class="<?php echo $tb_notifikasi_d_edit->RightColumnClass ?>"><div<?php echo $tb_notifikasi_d->lokasi_kerja->CellAttributes() ?>>
...
<input type="hidden" data-table="tb_notifikasi_d" data-field="x_lokasi_kerja" data-page="1" name="x_lokasi_kerja" id="x_lokasi_kerja" value="<?php echo ew_HtmlEncode($tb_notifikasi_d->lokasi_kerja->CurrentValue) ?>">
<?php echo $tb_notifikasi_d->lokasi_kerja->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tb_notifikasi_d->notif_active->Visible) { // notif_active ?>
	<div id="r_notif_active" class="form-group">
		<label id="elh_tb_notifikasi_d_notif_active" for="x_notif_active" class="<?php echo $tb_notifikasi_d_edit->LeftColumnClass ?>"><?php echo $tb_notifikasi_d->notif_active->FldCaption() ?></label>
		<div class="<?php echo $tb_notifikasi_d_edit->RightColumnClass ?>"><div<?php echo $tb_notifikasi_d->notif_active->CellAttributes() ?>>
<span id="el_tb_notifikasi_d_notif_active">
<select data-table="tb_notifikasi_d" data-field="x_notif_active" data-page="1" data-value-separator="<?php echo $tb_notifikasi_d->notif_active->DisplayValueSeparatorAttribute() ?>" id="x_notif_active" name="x_notif_active"<?php echo $tb_notifikasi_d->notif_active->EditAttributes() ?>>
<?php echo $tb_notifikasi_d->notif_active->SelectOptionListHtml("x_notif_active") ?>
</select>
</span>
<?php echo $tb_notifikasi_d->notif_active->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<input type="hidden" data-table="tb_notifikasi_d" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($tb_notifikasi_d->id->CurrentValue) ?>">
<?php if (!$tb_notifikasi_d_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tb_notifikasi_d_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tb_notifikasi_d_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
ftb_notifikasi_dedit.Init();
</script>
<?php
$tb_notifikasi_d_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tb_notifikasi_d_edit->Page_Terminate();
?>
