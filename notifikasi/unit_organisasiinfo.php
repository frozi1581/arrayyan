...
//
// Table class for unit_organisasi
//
class cunit_organisasi extends cTable {
	var $id;
	var $KD_ORG;
	var $NAMA_ORG;
	var $change_by;
	...
	function __construct() {
		...
		$this->TableVar = 'unit_organisasi';
		$this->TableName = 'unit_organisasi';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`unit_organisasi`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('unit_organisasi', 'unit_organisasi', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// KD_ORG
		$this->KD_ORG = new cField('unit_organisasi', 'unit_organisasi', 'x_KD_ORG', 'KD_ORG', '`KD_ORG`', '`KD_ORG`', 200, -1, FALSE, '`KD_ORG`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->KD_ORG->Sortable = TRUE; // Allow sort
		$this->fields['KD_ORG'] = &$this->KD_ORG;

		// NAMA_ORG
		$this->NAMA_ORG = new cField('unit_organisasi', 'unit_organisasi', 'x_NAMA_ORG', 'NAMA_ORG', '`NAMA_ORG`', '`NAMA_ORG`', 200, -1, FALSE, '`NAMA_ORG`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NAMA_ORG->Sortable = TRUE; // Allow sort
		$this->fields['NAMA_ORG'] = &$this->NAMA_ORG;

		// change_by
		$this->change_by = new cField('unit_organisasi', 'unit_organisasi', 'x_change_by', 'change_by', '`change_by`', '`change_by`', 200, -1, FALSE, '`change_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->change_by->Sortable = TRUE; // Allow sort
		$this->fields['change_by'] = &$this->change_by;

		// change_date
		$this->change_date = new cField('unit_organisasi', 'unit_organisasi', 'x_change_date', 'change_date', '`change_date`', ew_CastDateFieldForLike('`change_date`', 0, "DB"), 135, 0, FALSE, '`change_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`unit_organisasi`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "unit_organisasilist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "unit_organisasiedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "unit_organisasiadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("unit_organisasiview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("unit_organisasiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "unit_organisasiadd.php?" . $this->UrlParm($parm);
		else
			$url = "unit_organisasiadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("unit_organisasiedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("unit_organisasiadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("unit_organisasidelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->KD_ORG->setDbValue($rs->fields('KD_ORG'));
		$this->NAMA_ORG->setDbValue($rs->fields('NAMA_ORG'));
	...
	function RenderEditRow() {
		...
		$this->KD_ORG->EditValue = $this->KD_ORG->CurrentValue;
		...
		$this->NAMA_ORG->EditValue = $this->NAMA_ORG->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->KD_ORG->Exportable) $Doc->ExportCaption($this->KD_ORG);
					if ($this->NAMA_ORG->Exportable) $Doc->ExportCaption($this->NAMA_ORG);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->KD_ORG->Exportable) $Doc->ExportField($this->KD_ORG);
						if ($this->NAMA_ORG->Exportable) $Doc->ExportField($this->NAMA_ORG);
