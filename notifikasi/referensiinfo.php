...
//
// Table class for referensi
//
class creferensi extends cTable {
	var $id;
	var $id_ref;
	var $jenis_ref;
	var $index_ref;
	...
	function __construct() {
		...
		$this->TableVar = 'referensi';
		$this->TableName = 'referensi';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`referensi`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('referensi', 'referensi', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// id_ref
		$this->id_ref = new cField('referensi', 'referensi', 'x_id_ref', 'id_ref', '`id_ref`', '`id_ref`', 3, -1, FALSE, '`id_ref`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_ref->Sortable = TRUE; // Allow sort
		$this->id_ref->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_ref'] = &$this->id_ref;

		// jenis_ref
		$this->jenis_ref = new cField('referensi', 'referensi', 'x_jenis_ref', 'jenis_ref', '`jenis_ref`', '`jenis_ref`', 200, -1, FALSE, '`jenis_ref`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jenis_ref->Sortable = TRUE; // Allow sort
		$this->fields['jenis_ref'] = &$this->jenis_ref;

		// index_ref
		$this->index_ref = new cField('referensi', 'referensi', 'x_index_ref', 'index_ref', '`index_ref`', '`index_ref`', 200, -1, FALSE, '`index_ref`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->index_ref->Sortable = TRUE; // Allow sort
		$this->fields['index_ref'] = &$this->index_ref;

		// nama_referensi
		$this->nama_referensi = new cField('referensi', 'referensi', 'x_nama_referensi', 'nama_referensi', '`nama_referensi`', '`nama_referensi`', 200, -1, FALSE, '`nama_referensi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_referensi->Sortable = TRUE; // Allow sort
		$this->fields['nama_referensi'] = &$this->nama_referensi;

		// change_date
		$this->change_date = new cField('referensi', 'referensi', 'x_change_date', 'change_date', '`change_date`', ew_CastDateFieldForLike('`change_date`', 0, "DB"), 135, 0, FALSE, '`change_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// change_by
		$this->change_by = new cField('referensi', 'referensi', 'x_change_by', 'change_by', '`change_by`', '`change_by`', 200, -1, FALSE, '`change_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`referensi`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "referensiedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "referensiadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("referensiview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("referensiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "referensiadd.php?" . $this->UrlParm($parm);
		else
			$url = "referensiadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("referensiedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("referensiadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("referensidelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->jenis_ref->setDbValue($rs->fields('jenis_ref'));
		$this->index_ref->setDbValue($rs->fields('index_ref'));
		$this->nama_referensi->setDbValue($rs->fields('nama_referensi'));
	...
	function RenderEditRow() {
		...
		$this->jenis_ref->EditValue = $this->jenis_ref->CurrentValue;
		...
		$this->index_ref->EditValue = $this->index_ref->CurrentValue;
		...
		$this->nama_referensi->EditValue = $this->nama_referensi->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->jenis_ref->Exportable) $Doc->ExportCaption($this->jenis_ref);
					if ($this->index_ref->Exportable) $Doc->ExportCaption($this->index_ref);
					if ($this->nama_referensi->Exportable) $Doc->ExportCaption($this->nama_referensi);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->jenis_ref->Exportable) $Doc->ExportField($this->jenis_ref);
						if ($this->index_ref->Exportable) $Doc->ExportField($this->index_ref);
						if ($this->nama_referensi->Exportable) $Doc->ExportField($this->nama_referensi);
