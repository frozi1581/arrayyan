...
//
// Table class for tb_jurusan
//
class ctb_jurusan extends cTable {
	var $kd_jurusan;
	var $ket;
	var $created_by;
	var $created_date;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_jurusan';
		$this->TableName = 'tb_jurusan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_jurusan`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kd_jurusan
		$this->kd_jurusan = new cField('tb_jurusan', 'tb_jurusan', 'x_kd_jurusan', 'kd_jurusan', '`kd_jurusan`', '`kd_jurusan`', 200, -1, FALSE, '`kd_jurusan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_jurusan->Sortable = TRUE; // Allow sort
		$this->fields['kd_jurusan'] = &$this->kd_jurusan;

		// ket
		$this->ket = new cField('tb_jurusan', 'tb_jurusan', 'x_ket', 'ket', '`ket`', '`ket`', 200, -1, FALSE, '`ket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ket->Sortable = TRUE; // Allow sort
		$this->fields['ket'] = &$this->ket;

		// created_by
		$this->created_by = new cField('tb_jurusan', 'tb_jurusan', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 200, -1, FALSE, '`created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_by->Sortable = TRUE; // Allow sort
		$this->fields['created_by'] = &$this->created_by;

		// created_date
		$this->created_date = new cField('tb_jurusan', 'tb_jurusan', 'x_created_date', 'created_date', '`created_date`', ew_CastDateFieldForLike('`created_date`', 0, "DB"), 135, 0, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// last_update_by
		$this->last_update_by = new cField('tb_jurusan', 'tb_jurusan', 'x_last_update_by', 'last_update_by', '`last_update_by`', '`last_update_by`', 200, -1, FALSE, '`last_update_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_update_by->Sortable = TRUE; // Allow sort
		$this->fields['last_update_by'] = &$this->last_update_by;

		// last_update_date
		$this->last_update_date = new cField('tb_jurusan', 'tb_jurusan', 'x_last_update_date', 'last_update_date', '`last_update_date`', ew_CastDateFieldForLike('`last_update_date`', 0, "DB"), 135, 0, FALSE, '`last_update_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_jurusan`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kd_jurusan', $rs))
				ew_AddFilter($where, ew_QuotedName('kd_jurusan', $this->DBID) . '=' . ew_QuotedValue($rs['kd_jurusan'], $this->kd_jurusan->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kd_jurusan` = '@kd_jurusan@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->kd_jurusan->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@kd_jurusan@", ew_AdjustSql($this->kd_jurusan->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tb_jurusanedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tb_jurusanadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_jurusanview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_jurusanview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_jurusanadd.php?" . $this->UrlParm($parm);
		else
			$url = "tb_jurusanadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_jurusanedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_jurusanadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_jurusandelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kd_jurusan:" . ew_VarToJson($this->kd_jurusan->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kd_jurusan->CurrentValue)) {
			$sUrl .= "kd_jurusan=" . urlencode($this->kd_jurusan->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["kd_jurusan"]))
				$arKeys[] = $_POST["kd_jurusan"];
			elseif (isset($_GET["kd_jurusan"]))
				$arKeys[] = $_GET["kd_jurusan"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->kd_jurusan->CurrentValue = $key;
