...
//
// Table class for tb_bank
//
class ctb_bank extends cTable {
	var $kd_bank;
	var $ket;
	var $created_by;
	var $created_date;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_bank';
		$this->TableName = 'tb_bank';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_bank`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kd_bank
		$this->kd_bank = new cField('tb_bank', 'tb_bank', 'x_kd_bank', 'kd_bank', '`kd_bank`', '`kd_bank`', 200, -1, FALSE, '`kd_bank`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_bank->Sortable = TRUE; // Allow sort
		$this->fields['kd_bank'] = &$this->kd_bank;

		// ket
		$this->ket = new cField('tb_bank', 'tb_bank', 'x_ket', 'ket', '`ket`', '`ket`', 200, -1, FALSE, '`ket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ket->Sortable = TRUE; // Allow sort
		$this->fields['ket'] = &$this->ket;

		// created_by
		$this->created_by = new cField('tb_bank', 'tb_bank', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 200, -1, FALSE, '`created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_by->Sortable = TRUE; // Allow sort
		$this->fields['created_by'] = &$this->created_by;

		// created_date
		$this->created_date = new cField('tb_bank', 'tb_bank', 'x_created_date', 'created_date', '`created_date`', ew_CastDateFieldForLike('`created_date`', 0, "DB"), 135, 0, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// last_update_by
		$this->last_update_by = new cField('tb_bank', 'tb_bank', 'x_last_update_by', 'last_update_by', '`last_update_by`', '`last_update_by`', 200, -1, FALSE, '`last_update_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_update_by->Sortable = TRUE; // Allow sort
		$this->fields['last_update_by'] = &$this->last_update_by;

		// last_update_date
		$this->last_update_date = new cField('tb_bank', 'tb_bank', 'x_last_update_date', 'last_update_date', '`last_update_date`', ew_CastDateFieldForLike('`last_update_date`', 0, "DB"), 135, 0, FALSE, '`last_update_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_bank`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kd_bank', $rs))
				ew_AddFilter($where, ew_QuotedName('kd_bank', $this->DBID) . '=' . ew_QuotedValue($rs['kd_bank'], $this->kd_bank->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kd_bank` = '@kd_bank@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->kd_bank->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@kd_bank@", ew_AdjustSql($this->kd_bank->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tb_bankedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tb_bankadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_bankview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_bankview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_bankadd.php?" . $this->UrlParm($parm);
		else
			$url = "tb_bankadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_bankedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_bankadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_bankdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kd_bank:" . ew_VarToJson($this->kd_bank->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kd_bank->CurrentValue)) {
			$sUrl .= "kd_bank=" . urlencode($this->kd_bank->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["kd_bank"]))
				$arKeys[] = $_POST["kd_bank"];
			elseif (isset($_GET["kd_bank"]))
				$arKeys[] = $_GET["kd_bank"];
			else
