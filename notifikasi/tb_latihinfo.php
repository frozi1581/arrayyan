...
//
// Table class for tb_latih
//
class ctb_latih extends cTable {
	var $kd_latih;
	var $ket;
	var $created_by;
	var $created_date;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_latih';
		$this->TableName = 'tb_latih';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_latih`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kd_latih
		$this->kd_latih = new cField('tb_latih', 'tb_latih', 'x_kd_latih', 'kd_latih', '`kd_latih`', '`kd_latih`', 200, -1, FALSE, '`kd_latih`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_latih->Sortable = TRUE; // Allow sort
		$this->fields['kd_latih'] = &$this->kd_latih;

		// ket
		$this->ket = new cField('tb_latih', 'tb_latih', 'x_ket', 'ket', '`ket`', '`ket`', 200, -1, FALSE, '`ket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ket->Sortable = TRUE; // Allow sort
		$this->fields['ket'] = &$this->ket;

		// created_by
		$this->created_by = new cField('tb_latih', 'tb_latih', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 200, -1, FALSE, '`created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_by->Sortable = TRUE; // Allow sort
		$this->fields['created_by'] = &$this->created_by;

		// created_date
		$this->created_date = new cField('tb_latih', 'tb_latih', 'x_created_date', 'created_date', '`created_date`', ew_CastDateFieldForLike('`created_date`', 0, "DB"), 135, 0, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// last_update_by
		$this->last_update_by = new cField('tb_latih', 'tb_latih', 'x_last_update_by', 'last_update_by', '`last_update_by`', '`last_update_by`', 200, -1, FALSE, '`last_update_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_update_by->Sortable = TRUE; // Allow sort
		$this->fields['last_update_by'] = &$this->last_update_by;

		// last_update_date
		$this->last_update_date = new cField('tb_latih', 'tb_latih', 'x_last_update_date', 'last_update_date', '`last_update_date`', ew_CastDateFieldForLike('`last_update_date`', 0, "DB"), 135, 0, FALSE, '`last_update_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_latih`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kd_latih', $rs))
				ew_AddFilter($where, ew_QuotedName('kd_latih', $this->DBID) . '=' . ew_QuotedValue($rs['kd_latih'], $this->kd_latih->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kd_latih` = '@kd_latih@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->kd_latih->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@kd_latih@", ew_AdjustSql($this->kd_latih->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tb_latihedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tb_latihadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_latihview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_latihview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_latihadd.php?" . $this->UrlParm($parm);
		else
			$url = "tb_latihadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_latihedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_latihadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_latihdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kd_latih:" . ew_VarToJson($this->kd_latih->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kd_latih->CurrentValue)) {
			$sUrl .= "kd_latih=" . urlencode($this->kd_latih->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["kd_latih"]))
				$arKeys[] = $_POST["kd_latih"];
			elseif (isset($_GET["kd_latih"]))
				$arKeys[] = $_GET["kd_latih"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->kd_latih->CurrentValue = $key;
