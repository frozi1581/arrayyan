...
//
// Table class for tb_gas
//
class ctb_gas extends cTable {
	var $kd_gas;
	var $ket;
	var $created_by;
	var $created_date;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_gas';
		$this->TableName = 'tb_gas';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_gas`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kd_gas
		$this->kd_gas = new cField('tb_gas', 'tb_gas', 'x_kd_gas', 'kd_gas', '`kd_gas`', '`kd_gas`', 200, -1, FALSE, '`kd_gas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_gas->Sortable = TRUE; // Allow sort
		$this->fields['kd_gas'] = &$this->kd_gas;

		// ket
		$this->ket = new cField('tb_gas', 'tb_gas', 'x_ket', 'ket', '`ket`', '`ket`', 200, -1, FALSE, '`ket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ket->Sortable = TRUE; // Allow sort
		$this->fields['ket'] = &$this->ket;

		// created_by
		$this->created_by = new cField('tb_gas', 'tb_gas', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 200, -1, FALSE, '`created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_by->Sortable = TRUE; // Allow sort
		$this->fields['created_by'] = &$this->created_by;

		// created_date
		$this->created_date = new cField('tb_gas', 'tb_gas', 'x_created_date', 'created_date', '`created_date`', ew_CastDateFieldForLike('`created_date`', 0, "DB"), 135, 0, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// last_update_by
		$this->last_update_by = new cField('tb_gas', 'tb_gas', 'x_last_update_by', 'last_update_by', '`last_update_by`', '`last_update_by`', 200, -1, FALSE, '`last_update_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_update_by->Sortable = TRUE; // Allow sort
		$this->fields['last_update_by'] = &$this->last_update_by;

		// last_update_date
		$this->last_update_date = new cField('tb_gas', 'tb_gas', 'x_last_update_date', 'last_update_date', '`last_update_date`', ew_CastDateFieldForLike('`last_update_date`', 0, "DB"), 135, 0, FALSE, '`last_update_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// parent_kd_gas
		$this->parent_kd_gas = new cField('tb_gas', 'tb_gas', 'x_parent_kd_gas', 'parent_kd_gas', '`parent_kd_gas`', '`parent_kd_gas`', 200, -1, FALSE, '`parent_kd_gas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->parent_kd_gas->Sortable = TRUE; // Allow sort
		$this->fields['parent_kd_gas'] = &$this->parent_kd_gas;

		// old_kd_gas
		$this->old_kd_gas = new cField('tb_gas', 'tb_gas', 'x_old_kd_gas', 'old_kd_gas', '`old_kd_gas`', '`old_kd_gas`', 200, -1, FALSE, '`old_kd_gas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->old_kd_gas->Sortable = TRUE; // Allow sort
		$this->fields['old_kd_gas'] = &$this->old_kd_gas;

		// old_ket_gas
		$this->old_ket_gas = new cField('tb_gas', 'tb_gas', 'x_old_ket_gas', 'old_ket_gas', '`old_ket_gas`', '`old_ket_gas`', 200, -1, FALSE, '`old_ket_gas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->old_ket_gas->Sortable = TRUE; // Allow sort
		$this->fields['old_ket_gas'] = &$this->old_ket_gas;

		// status
		$this->status = new cField('tb_gas', 'tb_gas', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_gas`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kd_gas', $rs))
				ew_AddFilter($where, ew_QuotedName('kd_gas', $this->DBID) . '=' . ew_QuotedValue($rs['kd_gas'], $this->kd_gas->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kd_gas` = '@kd_gas@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->kd_gas->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@kd_gas@", ew_AdjustSql($this->kd_gas->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tb_gasedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tb_gasadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_gasview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_gasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_gasadd.php?" . $this->UrlParm($parm);
		else
			$url = "tb_gasadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_gasedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_gasadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_gasdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kd_gas:" . ew_VarToJson($this->kd_gas->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kd_gas->CurrentValue)) {
			$sUrl .= "kd_gas=" . urlencode($this->kd_gas->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["kd_gas"]))
				$arKeys[] = $_POST["kd_gas"];
			elseif (isset($_GET["kd_gas"]))
				$arKeys[] = $_GET["kd_gas"];
			else
	...
	function LoadListRowValues(&$rs) {
		...
		$this->parent_kd_gas->setDbValue($rs->fields('parent_kd_gas'));
		$this->old_kd_gas->setDbValue($rs->fields('old_kd_gas'));
		$this->old_ket_gas->setDbValue($rs->fields('old_ket_gas'));
	...
	function RenderEditRow() {
		...
		$this->parent_kd_gas->EditValue = $this->parent_kd_gas->CurrentValue;
		...
		$this->old_kd_gas->EditValue = $this->old_kd_gas->CurrentValue;
		...
		$this->old_ket_gas->EditValue = $this->old_ket_gas->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->parent_kd_gas->Exportable) $Doc->ExportCaption($this->parent_kd_gas);
					if ($this->old_kd_gas->Exportable) $Doc->ExportCaption($this->old_kd_gas);
					if ($this->old_ket_gas->Exportable) $Doc->ExportCaption($this->old_ket_gas);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->parent_kd_gas->Exportable) $Doc->ExportField($this->parent_kd_gas);
						if ($this->old_kd_gas->Exportable) $Doc->ExportField($this->old_kd_gas);
						if ($this->old_ket_gas->Exportable) $Doc->ExportField($this->old_ket_gas);
