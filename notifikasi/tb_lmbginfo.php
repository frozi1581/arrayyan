...
//
// Table class for tb_lmbg
//
class ctb_lmbg extends cTable {
	var $kd_lmbg;
	var $ket;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_lmbg';
		$this->TableName = 'tb_lmbg';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_lmbg`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kd_lmbg
		$this->kd_lmbg = new cField('tb_lmbg', 'tb_lmbg', 'x_kd_lmbg', 'kd_lmbg', '`kd_lmbg`', '`kd_lmbg`', 200, -1, FALSE, '`kd_lmbg`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_lmbg->Sortable = TRUE; // Allow sort
		$this->fields['kd_lmbg'] = &$this->kd_lmbg;

		// ket
		$this->ket = new cField('tb_lmbg', 'tb_lmbg', 'x_ket', 'ket', '`ket`', '`ket`', 200, -1, FALSE, '`ket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ket->Sortable = TRUE; // Allow sort
		$this->fields['ket'] = &$this->ket;

		// lokasi_pendidikan
		$this->lokasi_pendidikan = new cField('tb_lmbg', 'tb_lmbg', 'x_lokasi_pendidikan', 'lokasi_pendidikan', '`lokasi_pendidikan`', '`lokasi_pendidikan`', 200, -1, FALSE, '`lokasi_pendidikan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lokasi_pendidikan->Sortable = TRUE; // Allow sort
		$this->fields['lokasi_pendidikan'] = &$this->lokasi_pendidikan;

		// created_by
		$this->created_by = new cField('tb_lmbg', 'tb_lmbg', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 200, -1, FALSE, '`created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_by->Sortable = TRUE; // Allow sort
		$this->fields['created_by'] = &$this->created_by;

		// created_date
		$this->created_date = new cField('tb_lmbg', 'tb_lmbg', 'x_created_date', 'created_date', '`created_date`', ew_CastDateFieldForLike('`created_date`', 0, "DB"), 135, 0, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// last_update_by
		$this->last_update_by = new cField('tb_lmbg', 'tb_lmbg', 'x_last_update_by', 'last_update_by', '`last_update_by`', '`last_update_by`', 200, -1, FALSE, '`last_update_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_update_by->Sortable = TRUE; // Allow sort
		$this->fields['last_update_by'] = &$this->last_update_by;

		// last_update_date
		$this->last_update_date = new cField('tb_lmbg', 'tb_lmbg', 'x_last_update_date', 'last_update_date', '`last_update_date`', ew_CastDateFieldForLike('`last_update_date`', 0, "DB"), 135, 0, FALSE, '`last_update_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_lmbg`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kd_lmbg', $rs))
				ew_AddFilter($where, ew_QuotedName('kd_lmbg', $this->DBID) . '=' . ew_QuotedValue($rs['kd_lmbg'], $this->kd_lmbg->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kd_lmbg` = '@kd_lmbg@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->kd_lmbg->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@kd_lmbg@", ew_AdjustSql($this->kd_lmbg->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tb_lmbgedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tb_lmbgadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_lmbgview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_lmbgview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_lmbgadd.php?" . $this->UrlParm($parm);
		else
			$url = "tb_lmbgadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_lmbgedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_lmbgadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_lmbgdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kd_lmbg:" . ew_VarToJson($this->kd_lmbg->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kd_lmbg->CurrentValue)) {
			$sUrl .= "kd_lmbg=" . urlencode($this->kd_lmbg->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["kd_lmbg"]))
				$arKeys[] = $_POST["kd_lmbg"];
			elseif (isset($_GET["kd_lmbg"]))
				$arKeys[] = $_GET["kd_lmbg"];
			else
