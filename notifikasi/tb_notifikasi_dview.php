...
$tb_notifikasi_d_view = NULL; // Initialize page object first

class ctb_notifikasi_d_view extends ctb_notifikasi_d {
	...
	// Page object name
	var $PageObjName = 'tb_notifikasi_d_view';
	...
	function Page_Init() {
		...
		$this->status_notif->SetVisibility();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "tb_notifikasi_dlist.php"; // Return to list
			}
			if (@$_GET["employee_id"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "tb_notifikasi_dlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "tb_notifikasi_dlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($tb_notifikasi_d_view)) $tb_notifikasi_d_view = new ctb_notifikasi_d_view();

// Page init
$tb_notifikasi_d_view->Page_Init();

// Page main
$tb_notifikasi_d_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_notifikasi_d_view->Page_Render();
?>
...
var CurrentForm = ftb_notifikasi_dview = new ew_Form("ftb_notifikasi_dview", "view");

// Form_CustomValidate event
ftb_notifikasi_dview.Form_CustomValidate = 
...
ftb_notifikasi_dview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftb_notifikasi_dview.Lists["x_employee_id"] = {"LinkField":"x_employee_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x_lok_kerja","x_bagian",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"v_personal"};
ftb_notifikasi_dview.Lists["x_employee_id"].Data = "<?php echo $tb_notifikasi_d_view->employee_id->LookupFilterQuery(FALSE, "view") ?>";
ftb_notifikasi_dview.AutoSuggests["x_employee_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tb_notifikasi_d_view->employee_id->LookupFilterQuery(TRUE, "view"))) ?>;
ftb_notifikasi_dview.Lists["x_notif_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftb_notifikasi_dview.Lists["x_notif_active"].Options = <?php echo json_encode($tb_notifikasi_d_view->notif_active->Options()) ?>;
...
<div class="ewToolbar">
<?php $tb_notifikasi_d_view->ExportOptions->Render("body") ?>
<?php
	foreach ($tb_notifikasi_d_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $tb_notifikasi_d_view->ShowPageHeader(); ?>
<?php
$tb_notifikasi_d_view->ShowMessage();
?>
<form name="ftb_notifikasi_dview" id="ftb_notifikasi_dview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_notifikasi_d_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_notifikasi_d_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tb_notifikasi_d">
<input type="hidden" name="modal" value="<?php echo intval($tb_notifikasi_d_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($tb_notifikasi_d->employee_id->Visible) { // employee_id ?>
	<tr id="r_employee_id">
		<td class="col-sm-2"><span id="elh_tb_notifikasi_d_employee_id"><?php echo $tb_notifikasi_d->employee_id->FldCaption() ?></span></td>
...
<?php echo $tb_notifikasi_d->employee_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_notifikasi_d->bagian->Visible) { // bagian ?>
	<tr id="r_bagian">
		<td class="col-sm-2"><span id="elh_tb_notifikasi_d_bagian"><?php echo $tb_notifikasi_d->bagian->FldCaption() ?></span></td>
...
<?php echo $tb_notifikasi_d->bagian->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_notifikasi_d->lokasi_kerja->Visible) { // lokasi_kerja ?>
	<tr id="r_lokasi_kerja">
		<td class="col-sm-2"><span id="elh_tb_notifikasi_d_lokasi_kerja"><?php echo $tb_notifikasi_d->lokasi_kerja->FldCaption() ?></span></td>
...
<?php echo $tb_notifikasi_d->lokasi_kerja->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_notifikasi_d->notif_active->Visible) { // notif_active ?>
	<tr id="r_notif_active">
		<td class="col-sm-2"><span id="elh_tb_notifikasi_d_notif_active"><?php echo $tb_notifikasi_d->notif_active->FldCaption() ?></span></td>
		<td data-name="notif_active"<?php echo $tb_notifikasi_d->notif_active->CellAttributes() ?>>
<span id="el_tb_notifikasi_d_notif_active">
<span<?php echo $tb_notifikasi_d->notif_active->ViewAttributes() ?>>
<?php echo $tb_notifikasi_d->notif_active->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_notifikasi_d->status_notif->Visible) { // status_notif ?>
	<tr id="r_status_notif">
		<td class="col-sm-2"><span id="elh_tb_notifikasi_d_status_notif"><?php echo $tb_notifikasi_d->status_notif->FldCaption() ?></span></td>
		<td data-name="status_notif"<?php echo $tb_notifikasi_d->status_notif->CellAttributes() ?>>
<span id="el_tb_notifikasi_d_status_notif">
<span<?php echo $tb_notifikasi_d->status_notif->ViewAttributes() ?>>
<?php echo $tb_notifikasi_d->status_notif->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ftb_notifikasi_dview.Init();
</script>
<?php
$tb_notifikasi_d_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tb_notifikasi_d_view->Page_Terminate();
?>
