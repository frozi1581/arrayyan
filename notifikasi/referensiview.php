...
$referensi_view = NULL; // Initialize page object first

class creferensi_view extends creferensi {
	...
	// Page object name
	var $PageObjName = 'referensi_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "referensilist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "referensilist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "referensilist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($referensi_view)) $referensi_view = new creferensi_view();

// Page init
$referensi_view->Page_Init();

// Page main
$referensi_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$referensi_view->Page_Render();
?>
...
var CurrentForm = freferensiview = new ew_Form("freferensiview", "view");

// Form_CustomValidate event
freferensiview.Form_CustomValidate = 
...
freferensiview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $referensi_view->ExportOptions->Render("body") ?>
<?php
	foreach ($referensi_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $referensi_view->ShowPageHeader(); ?>
<?php
$referensi_view->ShowMessage();
?>
<form name="freferensiview" id="freferensiview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($referensi_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $referensi_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="referensi">
<input type="hidden" name="modal" value="<?php echo intval($referensi_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($referensi->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_referensi_id"><?php echo $referensi->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $referensi->id->CellAttributes() ?>>
<span id="el_referensi_id">
<span<?php echo $referensi->id->ViewAttributes() ?>>
<?php echo $referensi->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->id_ref->Visible) { // id_ref ?>
	<tr id="r_id_ref">
		<td class="col-sm-2"><span id="elh_referensi_id_ref"><?php echo $referensi->id_ref->FldCaption() ?></span></td>
...
<?php echo $referensi->id_ref->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->jenis_ref->Visible) { // jenis_ref ?>
	<tr id="r_jenis_ref">
		<td class="col-sm-2"><span id="elh_referensi_jenis_ref"><?php echo $referensi->jenis_ref->FldCaption() ?></span></td>
...
<?php echo $referensi->jenis_ref->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->index_ref->Visible) { // index_ref ?>
	<tr id="r_index_ref">
		<td class="col-sm-2"><span id="elh_referensi_index_ref"><?php echo $referensi->index_ref->FldCaption() ?></span></td>
...
<?php echo $referensi->index_ref->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->nama_referensi->Visible) { // nama_referensi ?>
	<tr id="r_nama_referensi">
		<td class="col-sm-2"><span id="elh_referensi_nama_referensi"><?php echo $referensi->nama_referensi->FldCaption() ?></span></td>
...
<?php echo $referensi->nama_referensi->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->change_date->Visible) { // change_date ?>
	<tr id="r_change_date">
		<td class="col-sm-2"><span id="elh_referensi_change_date"><?php echo $referensi->change_date->FldCaption() ?></span></td>
...
<?php echo $referensi->change_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($referensi->change_by->Visible) { // change_by ?>
	<tr id="r_change_by">
		<td class="col-sm-2"><span id="elh_referensi_change_by"><?php echo $referensi->change_by->FldCaption() ?></span></td>
...
<?php echo $referensi->change_by->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
