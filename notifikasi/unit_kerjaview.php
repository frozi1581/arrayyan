...
$unit_kerja_view = NULL; // Initialize page object first

class cunit_kerja_view extends cunit_kerja {
	...
	// Page object name
	var $PageObjName = 'unit_kerja_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "unit_kerjalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "unit_kerjalist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "unit_kerjalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($unit_kerja_view)) $unit_kerja_view = new cunit_kerja_view();

// Page init
$unit_kerja_view->Page_Init();

// Page main
$unit_kerja_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$unit_kerja_view->Page_Render();
?>
...
var CurrentForm = funit_kerjaview = new ew_Form("funit_kerjaview", "view");

// Form_CustomValidate event
funit_kerjaview.Form_CustomValidate = 
...
funit_kerjaview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $unit_kerja_view->ExportOptions->Render("body") ?>
<?php
	foreach ($unit_kerja_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $unit_kerja_view->ShowPageHeader(); ?>
<?php
$unit_kerja_view->ShowMessage();
?>
<form name="funit_kerjaview" id="funit_kerjaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($unit_kerja_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $unit_kerja_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="unit_kerja">
<input type="hidden" name="modal" value="<?php echo intval($unit_kerja_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($unit_kerja->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_unit_kerja_id"><?php echo $unit_kerja->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $unit_kerja->id->CellAttributes() ?>>
<span id="el_unit_kerja_id">
<span<?php echo $unit_kerja->id->ViewAttributes() ?>>
<?php echo $unit_kerja->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($unit_kerja->kode_unit_organisasi->Visible) { // kode_unit_organisasi ?>
	<tr id="r_kode_unit_organisasi">
		<td class="col-sm-2"><span id="elh_unit_kerja_kode_unit_organisasi"><?php echo $unit_kerja->kode_unit_organisasi->FldCaption() ?></span></td>
...
<?php echo $unit_kerja->kode_unit_organisasi->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($unit_kerja->kode_unit_kerja->Visible) { // kode_unit_kerja ?>
	<tr id="r_kode_unit_kerja">
		<td class="col-sm-2"><span id="elh_unit_kerja_kode_unit_kerja"><?php echo $unit_kerja->kode_unit_kerja->FldCaption() ?></span></td>
...
<?php echo $unit_kerja->kode_unit_kerja->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($unit_kerja->nama_unit_kerja->Visible) { // nama_unit_kerja ?>
	<tr id="r_nama_unit_kerja">
		<td class="col-sm-2"><span id="elh_unit_kerja_nama_unit_kerja"><?php echo $unit_kerja->nama_unit_kerja->FldCaption() ?></span></td>
...
<?php echo $unit_kerja->nama_unit_kerja->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($unit_kerja->change_by->Visible) { // change_by ?>
	<tr id="r_change_by">
		<td class="col-sm-2"><span id="elh_unit_kerja_change_by"><?php echo $unit_kerja->change_by->FldCaption() ?></span></td>
...
<?php echo $unit_kerja->change_by->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($unit_kerja->change_date->Visible) { // change_date ?>
	<tr id="r_change_date">
		<td class="col-sm-2"><span id="elh_unit_kerja_change_date"><?php echo $unit_kerja->change_date->FldCaption() ?></span></td>
...
<?php echo $unit_kerja->change_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
funit_kerjaview.Init();
</script>
<?php
$unit_kerja_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$unit_kerja_view->Page_Terminate();
?>
